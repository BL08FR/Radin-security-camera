 ...
 *  
 *  Modification to set maximum framerate and relay by Blaise Lapierre (08/12/2022).
 *  Check well in all the document ;).
 */
...
struct station stationList[] = {{"YOUR_WIFI_NETWORK_NAME","WIFI PASSWORD", true}};


// * You can extend the stationList[] above with additional SSID+Password pairs

//struct station stationList[] = {{"ESPcam", "1234567890", true},
//                                {"YOUR_WIFI_NETWORK_NAME", "WIFI PASSWORD", true},
//                                {"YOUR_WIFI_NETWORK_NAME 2", "WIFI PASSWORD 2", false}};
/*
 ...
 *  AccessPoint; 
 *
 *  Uncomment to enable AP mode (meaning the camera will create a wifi network to connect on); 
 *
 */
...
#define MINUTESTORESTART 15 //set how many minutes between each reboot

// RELAY CONFIGURATION
#define RELAYPIN 14 // define on what pin the relay action will be apply (with ai-thinker/HW-818 using pins 2-4-12-13-14-15 will forbid to use SD card,don't use pins listed in camera_pins.h)
#define RELAYPOLARITY 1 //define action by which relay is activated 1=+3.3v 0=GND
#define DAYSWITCHVALUE 50 // Auto relay ambient light threshold value, below, relay will be activated automatically (and opposite if higher)
long lightcheckdelay = 30000;// delay between each ambient light check (to don't enable/disable the relay too often)
