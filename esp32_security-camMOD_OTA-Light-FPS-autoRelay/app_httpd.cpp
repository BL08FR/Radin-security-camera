...
extern void setmaxFPS(int newVal);
extern void relayFunction(int newVal);
extern void lightvalue();

//MAX FPS variables
unsigned long lastStreamFrame;
extern int maxFPS;
...
static esp_err_t stream_handler(httpd_req_t *req){
    ...
    while(true){
      ...
      if(relay == 2 && firstcheck == 1 || millis() > lastlightcheck + lightcheckdelay){//if relay is set to auto and first loop in the while or delay elapsed
        lightvalue();//check light value
        lastlightcheck = millis();//set last test time
        if(firstcheck == 1){firstcheck = 0;}//if first test, set the next won't be
      }// autorelay stream request
    }

    streamsServed++;
    streamCount = 0;
    //if(relay == 2){relayFunction(0);}//relay stream end
    if (autoLamp && (lampVal != -1)) setLamp(0);
...
static esp_err_t cmd_handler(httpd_req_t *req){
    ...
    else if(!strcmp(variable, "idmaxFPS")){//maxfps
        maxFPS = constrain(val,1,30);
        setmaxFPS(maxFPS);
    }

    else if(!strcmp(variable, "idrelay")){//relay
        relay = constrain(val,0,2);
        relayFunction(relay);
        //Serial.print("relayFunction called: ");
    }
...
static esp_err_t status_handler(httpd_req_t *req){
    ...
    *p++ = '{';
    ...
    p+=sprintf(p, "\"idrelay\":%d,", relay);//relay
    p+=sprintf(p, "\"idmaxFPS\":%d,", maxFPS);//maxFPS
