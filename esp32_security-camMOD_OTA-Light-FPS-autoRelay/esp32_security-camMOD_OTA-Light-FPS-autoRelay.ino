...
// Primary config, or defaults.
    ...
    stationList[] = {{"ESP32-CAM-CONNECT","1234567890", true}};
#endif
...
void relayFunction(int newVal){
  Serial.print("relayFunction newVal: "); Serial.println(newVal);
  if(newVal == 0){
      Serial.println("Relay OFF!");
      if(RELAYPOLARITY == 0){digitalWrite(RELAYPIN, LOW);}//disable relay
      if(RELAYPOLARITY == 1){pinMode(RELAYPIN, OUTPUT);}//set relay pin as output (not to GND) to disable
  }
  if(newVal == 1){
  ...
  if(newVal == 2){Serial.println("Relay AUTO!");}
}//end relayfuntion
...
void printLocalTime(bool extraData=true) {
    struct tm timeinfo;
...
void setup() {
    ...
    if(psramFound()){
        ...
        config.fb_count = 2;// 2 give higher framerate (that I don't want)
    } else {
...
*/
    ...
    if(RELAYPOLARITY == 1){pinMode(RELAYPIN, OUTPUT); digitalWrite(RELAYPIN, LOW);}//set relay pin as output and to ground
...
void loop() {
    ...
    */
    ...
    //restart every xminutes
    if(AUTOREBOOT == 1 && millis() > (MINUTESTORESTART*(60*1000))){ESP.restart();}
}
